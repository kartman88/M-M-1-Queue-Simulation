...
from customer import arrival_rate, departure_rate, terminating as terminating_condition
...
if __name__ == "__main__":
    ...
    for i in range(simulation_number): 
        ...
        while (sim.n_departure < terminating_condition): #condizione di terminazione della simulazione
            if(starting_time==0 and sim.n_departure>terminating_condition//2):
                starting_time=sim.clock
        ...
        #Utilization
        end_time = sim.clock-starting_time
        ...
        # Calculate confidence intervals for expected response time
        response_time_mean = sum(response_time_list) / len(response_time_list)
        response_time_std = stats.sem(response_time_list)
        response_time_margin = response_time_std * stats.t.ppf((1 + confidence_level) / 2, len(response_time_list) - 1)
