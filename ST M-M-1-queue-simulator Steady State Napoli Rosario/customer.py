...
class Customer:
    ...
    def customer_arrival(self):
        ...
        if self.simulation.state == "free":
            ...
            if(self.simulation.n_departure>terminating//2): #se siamo a steady-state
            #Aggiornamento distribuzione
                ...
                if  not self.simulation.queue_distribution.get(self.simulation.queue_length-1):
                    self.simulation.queue_distribution[self.simulation.queue_length-1] = [0, self.simulation.clock, 0]
                else:   
    ...
    def customer_departure(self):
        ...
        if self.simulation.queue_length != 0:
            ...
            #Aggiornamento distribuzione
            if(self.simulation.n_departure==terminating//2-1):
                for i in range(0,self.simulation.queue_length):
                    self.simulation.queue_distribution[i]=[0, self.simulation.clock, 0]
